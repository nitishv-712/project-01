import Link from "next/link";

const topics = [
  "Power BI Desktop Interface",
  "Connecting to Data Sources",
  "Data Transformation with Power Query",
  "Data Modeling & Relationships",
  "DAX Formulas & Measures",
  "Building Interactive Dashboards",
  "Slicers, Filters & Drill-through",
  "Publishing Reports to Power BI Service",
];

const faqs = [
  { q: "Who is this course for?", a: "Data analysts, business professionals, and anyone who wants to create powerful dashboards and reports." },
  { q: "Do I need Excel knowledge first?", a: "Basic Excel knowledge is helpful but not required. The course is beginner-friendly." },
  { q: "Is Power BI free to use?", a: "Yes, Power BI Desktop is completely free. You only need a paid plan for sharing reports online." },
  { q: "Will I get a certificate?", a: "Yes, a certificate of completion is provided after finishing the course." },
];

export default function PowerBIMicroCourse() {
  return (
    <div className="min-h-screen">

      {/* Hero */}
      <section className="bg-gradient-to-br from-yellow-900 to-yellow-800 text-white py-20">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div className="space-y-5">
              <span className="inline-block bg-yellow-700 text-yellow-200 text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wider">Micro Course</span>
              <h1 className="text-4xl md:text-5xl font-bold leading-tight">
                Power BI <span className="text-orange-400">Micro Course</span>
              </h1>
              <p className="text-yellow-100 text-lg leading-relaxed">
                Learn to build stunning interactive dashboards and reports with Microsoft Power BI. From data import to publishing — covered step by step.
              </p>
              <div className="flex flex-wrap gap-4 text-sm text-yellow-200">
                <span className="flex items-center gap-1.5">
                  <svg className="w-4 h-4 text-orange-400" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" /></svg>
                  8–10 Hours
                </span>
                <span className="flex items-center gap-1.5">
                  <svg className="w-4 h-4 text-orange-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z" /></svg>
                  50+ Lessons
                </span>
                <span className="flex items-center gap-1.5">
                  <svg className="w-4 h-4 text-orange-400" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" /></svg>
                  Satish Dhawale
                </span>
              </div>
              <div className="flex flex-wrap gap-3 pt-2">
                <Link href="/mastery-courses"
                  className="bg-orange-500 text-white px-8 py-3.5 rounded-lg hover:bg-orange-600 transition font-bold">
                  Enroll Now
                </Link>
                <Link href="/mastery-courses" className="bg-white/10 text-white border border-white/20 px-8 py-3.5 rounded-lg hover:bg-white/20 transition font-semibold">
                  View All Courses
                </Link>
              </div>
            </div>
            <div className="hidden md:flex justify-center">
              <div className="w-72 h-72 bg-yellow-700/50 rounded-2xl flex items-center justify-center">
                <svg className="w-40 h-40 text-yellow-300 opacity-60" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M3 3h18v18H3V3zm2 2v14h14V5H5zm2 10h2v2H7v-2zm0-4h4v2H7v-2zm0-4h6v2H7V7zm8 8h2v2h-2v-2zm0-4h2v2h-2v-2z"/>
                </svg>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* What You'll Learn */}
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-10">
            <span className="text-orange-500 font-semibold text-sm uppercase tracking-wider">Curriculum</span>
            <h2 className="text-3xl font-bold text-gray-900 mt-2">What You'll Learn</h2>
          </div>
          <div className="grid md:grid-cols-2 gap-3 max-w-3xl mx-auto">
            {topics.map((topic) => (
              <div key={topic} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                <svg className="w-5 h-5 text-green-500 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                <span className="text-gray-700 text-sm">{topic}</span>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* FAQs */}
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-gray-900">FAQs</h2>
          </div>
          <div className="space-y-4">
            {faqs.map((faq) => (
              <div key={faq.q} className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
                <h3 className="font-bold text-gray-900 mb-2">{faq.q}</h3>
                <p className="text-gray-600 text-sm">{faq.a}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="py-14 bg-orange-500 text-white text-center">
        <div className="container mx-auto px-4 max-w-2xl">
          <h2 className="text-3xl font-bold mb-3">Start Building Dashboards Today</h2>
          <p className="text-orange-100 mb-8">Join thousands of students who have already mastered Power BI with Skill Course.</p>
          <Link href="/mastery-courses"
            className="inline-flex items-center gap-2 bg-white text-orange-600 px-10 py-4 rounded-lg hover:bg-orange-50 transition font-bold text-lg shadow-lg">
            Enroll Now
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          </Link>
        </div>
      </section>

    </div>
  );
}
